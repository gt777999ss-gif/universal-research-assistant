...
def workflow_markdown(report: Dict[str, Any]) -> str:
    ...
    if report.get("research_paper_signals"):
        lines.extend(["", "## Research and Paper Signals", ""])
        lines.extend(f"- **{item.get('title', '')}** ({item.get('date', '')}, {item.get('classification', '')}, {item.get('importance', '')}): {item.get('summary', '')} arXiv: {item.get('arxiv_url', '')} PDF: {item.get('pdf_url', '')}" for item in report["research_paper_signals"])
    return "\n".join(lines)
